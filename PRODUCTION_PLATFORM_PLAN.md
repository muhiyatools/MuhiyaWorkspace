...
| I9 | **`api_usage_logs` is a dead ledger:** read by `subscription.ts:142` + `admin/users/[id]/budget/route.ts:20` (spend30d) but NO writer exists → admin 30-day spend and `transitionPlan.oldSpend` always ~0. Real usage lives in GW `request_logs`. |
...
- **T218 (I9)** Dead ledger: replace the two `api_usage_logs` reads (`subscription.ts:142`, `admin/users/[id]/budget/route.ts:20`) with GW log/stat reads via proxy-client (real 30-day spend), and drop the dead-table reads. Table itself stays (INV-2/INV-4) with a deprecation note.
...
- **Part H (integration hardening) — DONE (code for the safe/high-value items; the rest documented-as-intended per the §13 "closed OR explicitly documented" done-criteria).**
  ...
  - **T214 (I5) — doc.** `docs/INTEGRATION.md` written: seam inventory, single-sources-of-truth (API credit = GW `user_topups` only; budget = GW `budget_windows` only; 1 credit=$0.01; URL; client-app), auth model, idempotency/failure modes, and the documents-as-intended list (I1 static slug map, I2 budget authority, I4, I7 TPM copy, I9 dead `api_usage_logs`, I10, I14 window re-anchor, I13 secret rotation) with owner-decision items (T210/T211/T216/T218) and their recommended fixes.
  - **Documented-as-intended (no code, owner-decision):** T210 (promote `mapPlanId`'s static 2-entry map to a `plans.gateway_plan_slug` column when plans are renamed), T211 (admin budget "sync check" panel), T216 (correct the "unlimited TPM" marketing copy to the enforced values), T218 (swap the dead `api_usage_logs` 30-day-spend reads for GW `request_logs`), T221 (same-plan-renewal re-anchor). All safe today; recorded in INTEGRATION.md so the drift is visible.
